...
class ConsumesUnreducedMetricsTest(absltest.TestCase):

  def test_recognizes_the_marker(self):
    self.assertTrue(
        utils.consumes_unreduced_metrics(utils.weighted_metric_mean)
    )

  def test_recognizes_legacy_names(self):
    def mean_of_means(values):
      return values

    self.assertTrue(utils.consumes_unreduced_metrics(mean_of_means))

  def test_rejects_a_plain_reducer(self):
    self.assertFalse(utils.consumes_unreduced_metrics(np.mean))
