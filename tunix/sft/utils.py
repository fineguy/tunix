...
def consumes_weighted_metrics(fn: Callable[..., Any]) -> Callable[..., Any]:
  """Marks a reducer as able to consume unreduced `WeightedMetric` values.

  Readers of a metrics buffer (`peft_trainer._write_metrics`, `mllog_utils`)
  have to know whether a buffered reducer can be handed raw `WeightedMetric`s
  or needs them reduced through `compute()` first -- doing the latter to a
  global weighted mean silently turns it into a mean of per-microbatch means.
  They historically answered that by matching `op.__name__` against a
  hardcoded list, which breaks quietly whenever a reducer is renamed or moved.
  This marker lets them ask the function instead.

  Args:
    fn: Reducer callable that accepts unreduced `WeightedMetric` objects.

  Returns:
    The input callable `fn` tagged with `consumes_weighted_metrics = True`.
  """
  setattr(fn, "consumes_weighted_metrics", True)
  return fn


# Reducers that took unreduced values before `consumes_weighted_metrics`
# existed. `global_weighted_mean` and `mean_of_means` live in `tunix.rl.common`
# and are still identified by name.
_LEGACY_UNREDUCED_REDUCER_NAMES = frozenset({
...
def consumes_unreduced_metrics(fn: Any) -> bool:
  """Reports whether a reducer can be handed unreduced `WeightedMetric`s."""
  if getattr(fn, "consumes_weighted_metrics", False):
    return True
  return getattr(fn, "__name__", "") in _LEGACY_UNREDUCED_REDUCER_NAMES


@consumes_weighted_metrics
