...
if [[ -z "$MODEL_ID" && -z "$MODEL_NAME" ]]; then
  ...
  case "${MODEL_NAME,,}" in
    *gemma-4*e2b*|*gemma4*e2b*) MODEL_ID=google/gemma-4-E2B-it ;;
    qwen3-*) MODEL_ID="Qwen/${MODEL_NAME}" ;;
    *)
      echo "Error: MODEL_ID is required for MODEL_NAME=${MODEL_NAME}."
      exit 1
      ;;
  esac
fi

case "${MODEL_NAME,,}:${MODEL_ID,,}" in
  *gemma-4*e2b*|*gemma4*e2b*)
    DEFAULT_ARTIFACT_DIR=gemma4_e2b_dist_frozenlake
    DEFAULT_TRAIN_MICRO_BATCH_SIZE=2
    DEFAULT_COMPUTE_LOGPS_MICRO_BATCH_SIZE=2
    DEFAULT_COMPUTE_LOGPS_CHUNK_SIZE=2048
    DEFAULT_ROLLOUT_MAX_CONCURRENCY=512
    DEFAULT_VLLM_MAX_NUM_SEQS=32
    DEFAULT_VLLM_MAX_NUM_BATCHED_TOKENS=8192
    DEFAULT_TRAINER_TPU_CHIPS=0,1
    DEFAULT_TRAINER_FSDP=2
    DEFAULT_TRAINER_TP=1
    DEFAULT_ROLLOUT_TPU_CHIPS=2,3
    DEFAULT_ROLLOUT_FSDP=1
    DEFAULT_ROLLOUT_TP=2
    DEFAULT_TPU_CHIPS_PER_HOST_BOUNDS=1,2,1
    ;;
  *)
    DEFAULT_ARTIFACT_DIR=qwen3_dist_frozenlake
    DEFAULT_TRAIN_MICRO_BATCH_SIZE=4
    DEFAULT_COMPUTE_LOGPS_MICRO_BATCH_SIZE=4
    DEFAULT_COMPUTE_LOGPS_CHUNK_SIZE=0
    DEFAULT_ROLLOUT_MAX_CONCURRENCY=256
    DEFAULT_VLLM_MAX_NUM_SEQS=64
    DEFAULT_VLLM_MAX_NUM_BATCHED_TOKENS=32768
    DEFAULT_TRAINER_TPU_CHIPS=0,1,2,3
    DEFAULT_TRAINER_FSDP=1
    DEFAULT_TRAINER_TP=4
    DEFAULT_ROLLOUT_TPU_CHIPS=4,5,6,7
    DEFAULT_ROLLOUT_FSDP=1
    DEFAULT_ROLLOUT_TP=4
    DEFAULT_TPU_CHIPS_PER_HOST_BOUNDS=1,4,1
    ;;
esac

ARTIFACT_ROOT=${ARTIFACT_ROOT:-"${REPO_ROOT}/artifacts/${DEFAULT_ARTIFACT_DIR}"}
...
TRAIN_MICRO_BATCH_SIZE=${TRAIN_MICRO_BATCH_SIZE:-$DEFAULT_TRAIN_MICRO_BATCH_SIZE}
COMPUTE_LOGPS_MICRO_BATCH_SIZE=${COMPUTE_LOGPS_MICRO_BATCH_SIZE:-$DEFAULT_COMPUTE_LOGPS_MICRO_BATCH_SIZE}
COMPUTE_LOGPS_CHUNK_SIZE=${COMPUTE_LOGPS_CHUNK_SIZE:-$DEFAULT_COMPUTE_LOGPS_CHUNK_SIZE}
...
ROLLOUT_MAX_CONCURRENCY=${ROLLOUT_MAX_CONCURRENCY:-$DEFAULT_ROLLOUT_MAX_CONCURRENCY}
...
TRAINER_TPU_CHIPS=${TRAINER_TPU_CHIPS:-$DEFAULT_TRAINER_TPU_CHIPS}
TRAINER_FSDP=${TRAINER_FSDP:-$DEFAULT_TRAINER_FSDP}
TRAINER_TP=${TRAINER_TP:-$DEFAULT_TRAINER_TP}
ROLLOUT_TPU_CHIPS=${ROLLOUT_TPU_CHIPS:-$DEFAULT_ROLLOUT_TPU_CHIPS}
ROLLOUT_FSDP=${ROLLOUT_FSDP:-$DEFAULT_ROLLOUT_FSDP}
ROLLOUT_TP=${ROLLOUT_TP:-$DEFAULT_ROLLOUT_TP}
TPU_CHIPS_PER_HOST_BOUNDS=${TPU_CHIPS_PER_HOST_BOUNDS:-$DEFAULT_TPU_CHIPS_PER_HOST_BOUNDS}
