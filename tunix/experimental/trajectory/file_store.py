...
import re
import types
from typing import Any, ClassVar, Final, Mapping, cast
...
class FileTrajectoryStore(store.TrajectoryStore[MetadataT, TrajectoryT]):
  ...
  def __init__(
      ...
      *,
      metadata_cls: type[MetadataT] = trajectory_lib.TrajectoryMetadata,
  ...
  def get_trajectories(self, trajectory_ids: list[str]) -> list[TrajectoryT]:
    ...
    for traj_id in trajectory_ids:
      ...
      if self._trajectory_cls is not None:
        ...
        trajs.append(cast(TrajectoryT, meta.create_trajectory(steps=steps)))
  ...
  def __enter__(self) -> "FileTrajectoryStore[MetadataT, TrajectoryT]":
